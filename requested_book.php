    ...
    if(isset($_GET['req']))
	{
		...
		if($total==3){
			...
			<script type="text/javascript">
				alert("You already selected three books.You must return one book first.");
			</script>
			<script>
		...
		else if(mysqli_num_rows($q2)!=0)
		{
			?>
			<script type="text/javascript">
				alert("You already selected this book.You must return it first.");
			</script>
			<script>
            	window.location="student_books.php";
        	</script>
		...
		while($row=mysqli_fetch_assoc($res))
		{
			...
			?>
		<script type="text/javascript">
			alert("book selected successfully.");
